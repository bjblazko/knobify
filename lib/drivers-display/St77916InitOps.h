...
// Board-specific ST77916 init sequence, ported verbatim from Waveshare's
// own official Arduino demo for this exact board (via the community
// mirror github.com/Sandjab/Waveshare-Knob,
// docs/demo-code/Arduino/examples/08_LVGL_Test/lcd_bsp.c) -- not
// reconstructed from memory or a summary, per
// docs/adr/0004-navigation-library-and-index-architecture.md. Encoded in
// Arduino_GFX's Arduino_DataBus::batchOperation() byte-operation format
// (BEGIN_WRITE/WRITE_C8_D8/WRITE_COMMAND_8/WRITE_BYTES/DELAY/END_WRITE)
// rather than Arduino_GFX's own built-in st77916_180_init_operations
// table, since that table's register values differ from Waveshare's
// (different panel calibration/batch) and this project trusts the
// verified-from-the-demo values instead.
...
inline const uint8_t kSt77916InitOps[] = {
    BEGIN_WRITE,
    // COLMOD (pixel format, 0x55 = 16bpp/RGB565) and MADCTL (RGB order)
    // aren't part of Waveshare's own vendor-specific command table --
    // their original esp_lcd_sh8601.c driver sends both automatically
    // before applying that table (panel_sh8601_init(), see ADR 0004).
    // KnobSt77916 doesn't replicate that automatic step, so they're
    // added explicitly here instead; without COLMOD the panel stays in
    // its power-on default pixel format and RGB565 data renders as
    // washed-out grey rather than color.
    ...
    WRITE_BYTES, 14,
    0xF0, 0x0A, 0x10, 0x09, 0x09, 0x36, 0x35, 0x33, 0x4A, 0x29, 0x15, 0x15,
    0x2E, 0x34,
    WRITE_COMMAND_8, 0xE1,
    WRITE_BYTES, 14,
    0xF0, 0x0A, 0x0F, 0x08, 0x08, 0x05, 0x34, 0x33, 0x4A, 0x39, 0x15, 0x15,
    0x2D, 0x33,
    ...
    WRITE_C8_D8, 0x36, 0x00,  // MADCTL: no 90-degree rotation.
    END_WRITE};
