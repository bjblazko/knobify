...
esp_err_t esp_lcd_new_panel_sh8601(const esp_lcd_panel_io_handle_t io, const esp_lcd_panel_dev_config_t *panel_dev_config, esp_lcd_panel_handle_t *ret_panel)
{
    ...
    // Adapted for this project's ESP-IDF version (bundled with the
    // PlatformIO espressif32 platform's Arduino core), which still uses
    // the older esp_lcd_color_space_t field rather than rgb_ele_order --
    // the upstream esp_lcd_sh8601 component (vendored from Waveshare's
    // demo) targets a newer ESP-IDF.
...
// This project's ESP-IDF version (4.4.x) renamed the interface field
// from `disp_on_off` to `disp_off`, with inverted boolean semantics
// (true = display off, not true = display on) -- the parameter here is
// named `off` and negated at the call site below to preserve this
// function's original on_off-flavored body.
static esp_err_t panel_sh8601_disp_on_off(esp_lcd_panel_t *panel, bool off)
{
    sh8601_panel_t *sh8601 = __containerof(panel, sh8601_panel_t, base);
    esp_lcd_panel_io_handle_t io = sh8601->io;
