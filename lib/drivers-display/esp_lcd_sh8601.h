...
// NOTE: the upstream macro (from Waveshare's demo, targeting a newer
// ESP-IDF) also sets `.flags = { .quad_mode = true }`, but that field
// doesn't exist on esp_lcd_panel_io_spi_config_t in the ESP-IDF version
// bundled with this project's Arduino core (4.4.x) -- omitted here.
// Quad-line transactions appear to be selected automatically from the
// SPI bus config (data0-data3 wired) in this IDF version rather than via
// an explicit flag, but this is unverified until tested on hardware.
